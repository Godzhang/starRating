var rating = (function(){
	...
	var Light = function(el, options){
		this.el = el;
		this.item = this.el.getElementsByTagName("li");
	...
	Light.prototype.lightOn = function(num){
		...
		for(var i = 0, len = this.item.length; i < len; i++){
			if(i < num){
				this.item[i].style['background-position'] = "0 -16px";
			}else{
				this.item[i].style['background-position'] = "0 0";
			}
		}
	}
	...
	LightHalf.prototype.lightOn = function(num){
		...
		if(isHalf){
			this.item[count].style['background-position'] = "0 -32px";
		}
	}
